...
fun minAndMax(number: Int): Int {
    ...
    if (number < 10 || number > 999999999) return 0

    // Ubah input menjadi string
    val str = number.toString()

    // Ubah string menjadi list of char
    val chars = str.toList()

    // Urutkan list of char dari kecil ke besar
    val sorted = chars.sorted()

    // Ambil angka terkecil dan terbesar dari list of char
    val min = sorted.first().toString().toInt()
    val max = sorted.last().toString().toInt()

    // Jumlahkan angka terkecil dan terbesar
    val sum = min + max
